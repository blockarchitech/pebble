/**
 * @description Pebble Spotify Route Handler
 */
...
router.get('/config', (req, res) => {
    ...
    fs.readFile('configs/spotify.html', 'utf8', (err, data) => {
        if (err) {
...
router.put('/analytics', (req, res) => {
    ...
    fs.readFile('analytics/spotify.json', 'utf8', (err, data) => {
        if (err) {
            // create the file
            fs.writeFile('analytics/spotify.json', JSON.stringify({
                created: new Date(),
                updated: new Date(),
                data: [req.body]
            }), (err) => {
            ...
            fs.writeFile('analytics/spotify.json', JSON.stringify(jsonData), (err) => {
                if (err) {
                    res.status(202).send('analytics is dead.');
                }
            });
